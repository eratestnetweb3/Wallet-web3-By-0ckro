...
function App() {
  ...
  const resetWallet = () => { if (!window.confirm('Reset wallet akan menghapus vault dan wallet lama dari perangkat ini. Tanpa seed phrase atau backup, aset tidak dapat dipulihkan. Lanjutkan?')) return; localStorage.removeItem(STORAGE_KEY); setVault(null); setKeypair(null); setPassword(''); setBalance(null); setNotice('Vault dihapus. Buat wallet baru atau pulihkan dari backup yang aman.') }
  ...
  return <div className={`app ${theme}`}>
    ...
    <main><section className="hero"><div><p className="eyebrow">SELF-CUSTODY / {network.toUpperCase()}</p><h1>Your wallet,<br /><span>in full orbit.</span></h1><p className="lead">A calm, local-first home for your digital assets. Your keys stay encrypted on this device.</p></div><div className="orbit-card"><div className="planet" /><span className="orbit orbit-one" /><span className="orbit orbit-two" /><span className="orbit orbit-three" /></div></section>
      {!vault ? <section className="panel onboarding"><div><p className="eyebrow">FIRST FLIGHT</p><h2>Create your local vault</h2><p>No seed phrase or private key is sent to a server. This foundation uses AES-GCM and PBKDF2 in your browser.</p></div><div className="form-row"><input type="password" placeholder="Vault password (8+ characters)" value={password} onChange={e => setPassword(e.target.value)} /><button onClick={createWallet}>Create wallet</button></div></section> : !keypair ? <section className="panel onboarding"><div><p className="eyebrow">VAULT LOCKED</p><h2>Welcome back</h2><p>Unlock your encrypted local vault to view the wallet address and balance. Forgot the password? Resetting permanently deletes this local vault.</p></div><div className="form-row"><input type="password" placeholder="Vault password" value={password} onChange={e => setPassword(e.target.value)} /><button onClick={unlock}>Unlock</button><button className="danger-button" onClick={resetWallet}>Reset wallet</button></div></section> : <>
        ...
        {tab === 'home' && <><section className="grid"><div className="panel balance-card"><p className="eyebrow">TOTAL PORTFOLIO</p><div className="balance">{balance === null ? '—' : balance.toFixed(4)} <small>SOL</small></div><div className="address">{short(address)} <button onClick={() => navigator.clipboard.writeText(address)}>Copy</button></div><div className="stat"><span>Network</span><b>{network === 'devnet' ? 'Development' : 'Production'}</b></div></div><div className="panel safety"><p className="eyebrow">SECURITY STATUS</p><h3>Local vault active</h3><p>Encrypted with AES-GCM. Signing is designed to happen locally after a human-readable transaction preview.</p><div className="shield">✓</div></div></section><section className="panel asset-panel"><div className="section-heading"><div><p className="eyebrow">YOUR ASSETS</p><h2>Portfolio overview</h2></div><span className="live-badge">● LIVE RPC</span></div><div className="asset-row"><div className="asset-icon">S</div><div className="asset-name"><strong>Solana</strong><span>SOL · {network}</span></div><div className="asset-amount"><strong>{balance === null ? '—' : balance.toFixed(4)} SOL</strong><span>Native asset</span></div></div><div className="empty-assets">Token balances, NFTs, and price history will appear here as read-only integrations are added.</div></section></>}
